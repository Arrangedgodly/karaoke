...
- **The card meters (Advanced, 2026-09-03 owner direction):** the board's detail layer — a 6px lamp strip along every chain card's foot, in the VU ladder's own green/amber/red. The BAR is the level LEAVING that card; the TICK is the level that ARRIVED at it, so the distance between them is the effect that card is having, shown as two real measurements rather than a derived reduction number this app cannot defend. A bypassed card reads its input unchanged, because that is what is physically leaving its seat. Fed by `StageTaps.feedChain` — the chain's boundaries, nodes+1 of them, read off the taps the chevrons already use, so the two surfaces can never disagree about board order or about a level. Unlit glass behind the bar (unlike the nameplate matrix, whose unlit cells must stay legible): this IS a lamp bar read by its lit length. `aria-hidden`, fixed height, ballistics on the boundaries themselves so a card's bar and its neighbour's tick can never disagree about the same measurement. A folded card shows none — there is no field to meter.
- **The base row (same round):** MIC IN and OUT used to line the board top and bottom, a full-width row each, costing ~90px on a console whose whole point is that it never scrolls. With the per-card strips carrying the reading, those two become what they always really were — the pinned SAFETY ANCHOR — and share ONE compact row under the board (~51px for both). They stay panel children, never inside `#chain-canvas`, so output ground truth still never depends on scroll or collapse and still never dims, and Simple's own strips are untouched: the safety floor's "available in both views" holds.

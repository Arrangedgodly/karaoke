...
**The Power-Up Exception** (2026-09-02, owner-approved; extended 2026-09-03). The engine's cold start is the ONE sequence permitted past the 150–250ms answer law, and it now has three parts: the meters' lamp test, the gated face's wake (the print rising to full while the hatch peels off toward OUT, 420ms), and the nameplate matrix's DISPLAY SELF-TEST — a wipe that crosses the word lighting every cell, a 120ms hold with the whole name lit, and a drain back to print. The self-test runs on the lamp test's own clock (260/120/320ms), so the ladders and the name wake as one machine rather than as three animations; it is not a fourth exception, it is this one, on the surface that became a lamp. The wordmark therefore takes the lamp test's three honesty rules verbatim: the drawn cell height is `max(real, sweep)`, so the proof can only ADD light and can never under-report a real signal; it touches LIGHTING alone, and clip colour still comes from the real signal only, so a sweep can never claim a clip; and it runs once per engine start. All three parts run at setup, never during a show; they fill a wait that already exists and gate nothing — the gate class is off before the wake's first frame, so the surface is interactive throughout. Every part is `prefers-reduced-motion` guarded and degrades to a state rather than a motion (the meters and the word both light full-scale for a 200ms hold and release). Everything else on this instrument still answers in 150–250ms, and page LOAD choreography stays banned — this is a state answer to a deliberate human action, not an entrance.

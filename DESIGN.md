...
**The Power-Up Exception** (2026-09-02, owner-approved; extended 2026-09-03). The engine's cold start is the ONE sequence permitted past the 150–250ms answer law, and it now has three parts: the meters' lamp test, the gated face's wake (the print rising to full while the hatch peels off toward OUT, 420ms), and the nameplate matrix's DISPLAY SELF-TEST — a wipe that crosses the word lighting every cell, a 120ms hold with the whole name lit, and a drain back to print. The self-test runs on the lamp test's own clock (260/120/320ms), so the ladders and the name wake as one machine rather than as three animations; it is not a fourth exception, it is this one, on the surface that became a lamp. The wordmark therefore takes the lamp test's three honesty rules verbatim: the drawn cell height is `max(real, sweep)`, so the proof can only ADD light and can never under-report a real signal; it touches LIGHTING alone — the sweep lights the full ladder, red row included, because proving every segment is what a lamp test IS and is exactly what the meters' own sweep does, while the clip LATCH, every readout and every aria value stay on the real feed (the wordmark has none of those to touch, which is why it can hold this rule trivially); and it runs once per engine start. All three parts run at setup, never during a show; they fill a wait that already exists and gate nothing — the gate class is off before the wake's first frame, so the surface is interactive throughout. Every part is `prefers-reduced-motion` guarded and degrades to a state rather than a motion (the meters and the word both light full-scale for a 200ms hold and release). Everything else on this instrument still answers in 150–250ms, and page LOAD choreography stays banned — this is a state answer to a deliberate human action, not an entrance.
...
- **How it lives:** `src/signal-lamps.js` lights the cells from the baseline up, column by column across the word, from the same final-output window the scope slots read — one feed, every live surface, no second measurement. Unlit cells print bright (`--pm-print-hi`, NOT the meters' unlit glass: a dark cell is right for a lamp bar read by its lit length and wrong for a product name that must be legible on a dead engine in a dark room), so the signal can only ADD light and can never change the resting face. Lit cells take the VU LADDER ITSELF (2026-09-03): colour belongs to the ROW, not to the level, so the word is 47 little ladders standing side by side and a loud syllable pushes red through the tops of the letters exactly as it pushes red up the OUT bar. A row's BODY ink is the zone of its own upper edge — the level at which it is fully lit — so the ladder runs green (rows 0–3), amber (4–5), red (6) from the baseline up. Seven rows over 60 dB is coarse, though, and the meters' edges land mid-row: position ink ALONE read a whole zone cold, showing green from -20 all the way to -13 dBFS while the bars beside it were already amber — which is exactly where speech sits, so the word looked green through an entire performance. The TIP therefore reports the level: the top lit row takes the column's measured zone, computed the way the meters compute theirs, so the word flips zone on the same side of -20 and -6 that the bars do. The body is the ladder; the tip is the reading. A row raised by the power-up sweep rather than by signal keeps position ink — a proof reports no level. Each column also carries the house activity-light ballistics (instant attack, 24 dB/s fall): without them a syllable's peak painted for a single frame and the eye never caught it, which was the other half of why the word read green against held meter bars. The lit height reads that same dB scale (`envelopeUnit`, SCALE_MIN..0 dBFS) rather than a sqrt lift on linear amplitude: the envelope carries linear peak and every other lamp on this machine reads dB, which is why a voice at -30 dBFS — a third of the way up a meter — was rendering as a twentieth of full amplitude and leaving the word dark through most of real speech. The scopes share the mapping for their INK; their trace HEIGHT stays linear, because that silhouette is a literal picture of the waveform and stretching it would be a lie about the shape. This redistributes the range and never adds gain — silence still lights nothing. Engine stop repaints the resting face — a frozen last frame would claim a signal that is no longer there — and bypass flattens it by itself, because the output it reads is what bypass gates.
...
### Meters
- **MIC IN / OUT units + the VU corner:** 96×26px canvas lamp bars (unlit glass, green→amber→clip stops, peak tick, clip latch dot) with mono dB readouts at the 12px tier. In Advanced they are fixed endcaps around the scrolling chain: MIC IN at the left and OUT at the right. They read as the first and last cells of one horizontal workflow while output ground truth stays visible at every scroll position and never dims, including while bypassed. Simple mounts the same live units around its current-sound stage. Ballistics are per-frame canvas draws: functional motion, live under reduced-motion by construction.
- **The card meters (Advanced, 2026-09-03 live direction):** the board's detail layer is an explicit two-row IN/OUT comparison machined into every chain card's bottom groove, not mounted as another display. The accepted Floor Groove treatment is 6px tall, inset 6px from both card edges, and held at 50% intensity; the upper trace is IN and the lower trace is OUT, with no repeated labels competing with the controls. Each trace uses the VU ladder's green/amber/red stops; IN is quieter and OUT leads without hiding the before/after pair. The traces follow RMS from the real boundary taps because RMS exposes compression, EQ, and ambience changes that transient peaks can make look identical. A bypassed card shows equal IN and OUT because that is what physically leaves its seat. `StageTaps.feedChain` supplies the chain's nodes+1 boundaries, and ballistics run on those shared boundaries so one card's OUT cannot disagree with its neighbor's IN. The comparison is `aria-hidden`; the endpoint meters keep the accessible numeric reading. A folded card shows none because there is no field to meter.
- **The fixed horizontal workflow (same round):** the endpoint hosts and the scrolling canvas form one three-cell grid. The canvas keeps its existing drag, reorder, and scroll contract in the middle; the safety meters stay outside that scroll in the fixed endcaps. Drawn chevrons align along the top reading line so the route reads MIC IN → effects → OUT instead of dropping to a detached footer. Simple's own strips are untouched, so the safety floor remains available in both views.
- **The output scope band (Advanced, 2026-09-03 live round):** the Simple stage's scope as a second VIEW on the voice deck — the same final-output tap, the same 42.7 ms window — as a fixed 3.5rem band between the horizontal workflow and the signal-order strip below. It PRINTS rather than mounts: Advanced's live-signal idiom is already light ON the faceplate (the chevron lamps between cards), not light inside a display window, so the band takes the signal-order strip's own vocabulary — machined lip over the top cut, groove-cut below, the chassis showing through behind the trace — instead of the register's inset ground. `src/signal-lamps.js` BUILDS Simple's slot and ADOPTS every other one by canvas class, painting all of them from ONE feed, so two views can never disagree and a slot that is off screen costs a zero-box test and nothing else. `aria-hidden` like its twin (the fixed OUT endcap keeps the numbers), no accent anywhere, and `display: none` before Start: the band has no cold reading, so it arrives with the live face and leaves with it.

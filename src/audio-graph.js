  ...
  function buildGraph(model) {
    ...
    if (pendingRewireTimer !== null) {
      ...
      if (pendingRewireState) {
        disposeStagedNodes(
          pendingRewireState.model,
          pendingRewireState.resolvedNodes,
          pendingRewireState.oldNodeInstances
        );
        pendingRewireState.resolve({
          committed: false,
          canceled: true,
        ...
        try {
          ...
          try {
            ...
            var restoreMuted =
              ...
              typeof window.MeterTaps.isTripped === 'function' &&
              window.MeterTaps.isTripped();
            var restoreTarget = restoreMuted
              ? 0
              : (window.AudioBypass && window.AudioBypass.isEngaged()) ? 0 : 1.0;
            rampGateTo(gate, restoreTarget, audioContext);

...
//
// PEN-1 ORDER (cycle 4, batch PR): the 20 entries are sequenced
// GENRE-FIRST — the six GEN-1 genre candidates in sketch order
// (Metal → Rap → R&B/Soul → Country → Dance/EDM → Musicals), then the
// seed Jazz genre entry, the three vibe entries, and the cleanup
// use-case entry, and only then the nine gags (seed chipmunk /
// deep-voice / radio, the three re-authors, the three new cells) — so
// the user's audition validates domain content before gags. A pure data
// move (no entry bytes changed); the committed conformance pass in
// tests/test-factory-presets-policy.js section F pins the order so the
// batch cannot regress silently.

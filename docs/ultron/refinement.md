...
- **Residuals (disclosed)**: help discovery remains hover/focus-only
  ...
  Drive/Tone read "0.25%"/"0.7%" until entry 4 lands (it has — see entry
  4 below; the help lines deliberately avoid referencing the readout
  numbers so both conventions read true).
...
### Entry 4 — `$impeccable polish` (P3-4 distortion Drive/Tone readout) — done-pending-approval

- **Files changed**: `src/param-controls.js` (formatValue + its three
  call sites in the one file — the single rendering path for every
  param readout), `src/node-distortion.js` (drive/tone paramSpec entries
  gain one field each + comment),
  `tests/test-palette-cards-cycle3.js` (+34: new section K). No CSS, no
  markup, no audio-graph/MCP/preset files — the mono register is a CSS
  token already worn by the same `.param-value` span, untouched.
- **Mechanism**: DISPLAY-ONLY normalization through a data-driven
  presentation seam, the critique's first fix option as the user
  approved. Drive/Tone declare `displayScale: 100` in their paramSpec
  (a UI-only field in the label/step family — deliberately NOT mirrored
  in mcp-tools.js's NODE_REGISTRY_SNAPSHOT); the generic formatter
  multiplies ONLY the rendered string (with a 1e6 round to kill binary
  float tails: 0.33 × 100 === 33.000000000000004). Everything upstream
  of the string keeps the internal 0..1 scale byte-for-byte: slider
  min/max/step/default, the parsed model value, AudioGraph bookkeeping,
  preset serialization (a saved drive 0.25 still means the same sound),
  the audio mapping (driveToGain/toneToHz untouched), and the agent
  set_param contract (mcp-tools validates against the unchanged min/max
  0..1; the capabilities readout keeps truthfully publishing 0..1 — it
  states 0–1, so it stays truthful by NOT changing). The readout now
  reads "25%"/"70%" — the exact string shape Mix "30%" set, in the same
  mono tabular register (strictly shorter than "0.25%", so the fader
  row rhythm cannot regress); README.md's own operator disclosure
  ("25 % is a light warmth, 100 % is a full roar") already spoke in
  0–100 terms, so the card and the README now agree.
- **Sweep (the critique's "other params" question)**: audited every
  paramSpec across all ten types. Distortion drive/tone were the ONLY
  0–1-with-% mismatches. Every other % param is already 0–100 integer
  scale (delay Feedback 0–90/Mix, reverb/chorus/autotune Mix 0–100 —
  render "25%"/"30%"/… raw, unchanged); gate threshold/floor (dB),
  attack/release (s), chorus Depth (ms)/Rate (Hz), autotune Retune
  (ms), compressor ratio (:1)/attack/release render in their documented
  units. No other fix needed. The sweep is now suite-enforced: section
  K's registry gate fails a future 0..1 % param that lacks displayScale
  (and a 0–100 % param that gains one), and K's rendered ten-type scan
  fails any "0.X%" readout anywhere on the surface.
- **Evidence**: `tests/test-palette-cards-cycle3.js` section K (34 new
  checks): readouts "25%"/"70%"/"-3 dB" at defaults with the slider
  still min 0/max 1/step 0.01/value 0.25; human drag path at 0/0.5/1/
  back-to-0.25 reads "0%"/"50%"/"100%"/"25%" while the committed model
  value stays exactly 0/0.5/1/0.25; the float-noise case 0.33 reads
  "33%" and commits 0.33; preset restore (drive 0.42/tone 0.06) reads
  "42%"/"6%" with getCurrentModel (the persistence/preset read)
  unchanged; the agent set_param canvas half (ChainCanvas.
  updateNodeParam — the exact call MCP's parameter-only fast path makes)
  applies 0.5 → span "50%", slider at 0.5, model 0.5, autosave 0.5; the
  registry-wide % gate; and the mounted-all-ten "0.X%" scan (7 % readouts
  surface-wide). MCP-side 0..1 semantics additionally pinned by the
  existing test-mcp-tools-cycle3.js C8 (set_param drive 0.9 → model 0.9),
  green in the run below. Full suite **23/23 files / 1896 checks green**.
  Render-level check (headless Chrome over http, real index.html, real
  chip clicks): distortion card reads Drive "25%"/Tone "70%"/Output
  "-3 dB" with slider 0–1/0.01; input events at 0/0.5/1 → "0%"/"50%"/
  "100%" and model 0.25 restored after drag-back; updateNodeParam 0.5 →
  "50%" + model 0.5; chorus Mix "30%" (convention reference), Depth
  "3 ms", Rate "1.5 Hz"; the value span's computed style is still the
  mono readout register (ui-monospace stack, tabular-nums, 11.2px);
  every % readout on the live surface is 0.X-free.
- **Residuals (disclosed)**: none for this entry — the change is a
